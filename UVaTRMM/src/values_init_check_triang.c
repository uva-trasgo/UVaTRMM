...
int main( int argc, char *argv[] ) {
	...
	if ( strlen( argv[1] ) > 32 ) {
		fprintf( stderr, "Error: file_prefix should be 32 chars at most\n" );
	...
	/*	for ( i=0; i<rows; i++ ) {
		...
		/* Fill A with consecutive values */
		for ( i=0; i<rows; i++ ) {
			for ( j=0; j<columns; j++ ) {
				// Triangular matrix
				if ( j > 0 + i ) mat_A[ i * columns + j ] = 0;
				else mat_A[ i * columns + j ] = ( ( (i+0) * columns ) + j ) % VALUES_LIMIT + 1;
			}
		}
	//}
	/*	
	...
	*/
	/*if ( init_scheme == INIT_B_ID ) {
		...
		/* Fill B with consecutive values and C with 0s */
		for ( i=0; i<rows; i++ ) {
			for ( j=0; j<columns; j++ ) {
				mat_C[ i * columns + j ] = 0;
